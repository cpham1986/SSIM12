function elevationMatrix = elevationFunction(stationConstant)
    ...
    if(stationConstant == 2)
        ...
        elevationMatrix(3806:5356,3) = 0; % Set scan direction to FRO for fro scan
        ...
        elevationMatrix(3806:5455, 4) = 1; % Set scanning beam start to 1 for the FRO scan
        ...
        elevationMatrix(1856:5055,5) = 1; % Set antenna select to 5 for the normal scanning beam antenna
        elevationMatrix(1856:5055,6) = 0; % Set antenna select to 5 for the normal scanning beam antenna
        elevationMatrix(1856:5055,7) = 1; % Set antenna select to 5 for the normal scanning beam antenna
    
        elevationMatrix(5056:5600, 5) = 0; % Set antenna select to 0 for the default antenna
        elevationMatrix(5056:5600, 6) = 0; % Set antenna select to 0 for the default antenna
        elevationMatrix(5056:5600, 7) = 0; % Set antenna select to 0 for the default antenna
        ...
        elevationMatrix(5056:5119,8) = 1; % Set antenna select read to 1 to set the antenna back to the default antenna
    end
end